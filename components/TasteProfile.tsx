  ...
  return (
    <>
      ...
      <button
        ...
        className={`fixed right-5 top-1/2 z-40 -translate-y-1/2 flex h-14 w-14 items-center justify-center rounded-full border border-white/40 bg-white/25 text-2xl text-[#5b351f] shadow-lg backdrop-blur-md transition-all duration-300 hover:bg-white/40 ${showRec ? "pointer-events-none opacity-0" : "opacity-100"}`}
      >
        ›
      </button>
      ...
      {/* 추천 콘텐츠 슬라이드 패널 */}
      <div
        className={`fixed inset-0 z-50 flex flex-col bg-[#fff8eb] transition-transform duration-500 ease-in-out ${showRec ? "translate-x-0" : "translate-x-full"}`}
      >
        {/* 패널 헤더 */}
        <div className="sticky top-0 z-10 border-b border-[#8a5a2f]/15 bg-[#fff8eb]/90 backdrop-blur">
          <div className="mx-auto flex max-w-6xl items-center gap-4 px-5 py-4">
            <button
              type="button"
              onClick={() => setShowRec(false)}
              aria-label="취향 리포트로 돌아가기"
              className="flex h-10 w-10 items-center justify-center rounded-full border border-[#8a5a2f]/20 bg-[#fbf1dd] text-xl text-[#8a5a2f] transition hover:bg-[#ead7b8]"
            >
              ‹
            </button>
            <h2 className="text-lg font-bold text-[#5b351f]">추천 콘텐츠</h2>
          </div>
        </div>
            ...
            <div className="warm-panel rounded-[28px] p-7 md:p-10">
              <div className="flex items-start justify-between gap-4">
                <div>
                  ...
                  <h3 className="mt-3 text-2xl font-black text-[#3f2a1d]">추천 콘텐츠</h3>
